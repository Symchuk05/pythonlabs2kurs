...
### Де знаходиться
📁 `snake_game/exceptions.py`

### Що реалізовано
- **GameError** - базовий клас для всіх ігрових помилок
- **CollisionError** - зіткнення змійки
- **InvalidMoveError** - невірний рух
- **LevelLoadError** - помилка завантаження рівня
- **SaveGameError** - помилка збереження
- **InvalidPositionError** - невірна позиція
...
### Де використовується
- `engine.py` - рядки 192, 195 (викидання)
- `engine.py` - рядки 124, 180, 295 (ловлення)
...
### Де знаходиться
📁 `snake_game/utils/generators.py`
...
**Де використовується:** `engine.py`, рядок 335
...
**Де використовується:** `engine.py`, рядок 405
...
### Де знаходиться
📁 `snake_game/utils/decorators.py`
...
**Де використовується:** `engine.py`, рядок 147
...
**2. `__getitem__`** - доступ за індексом
```python
def __getitem__(self, index: int) -> Position:
    return self.__body[index]
...
**3. `__contains__`** - перевірка наявності
```python
...
**5. `__reversed__`** - зворотна ітерація
```python
...
**Де використовується:** `game_field.py`, рядок 71 (for pos in snake)
...
### Структура тестів
- `test_position.py` - 15 тестів для Position
- `test_snake.py` - 16 тестів для Snake
- `test_food.py` - 12 тестів для Food
- `test_decorators.py` - 5 тестів для декораторів
- `test_exceptions.py` - 9 тестів для винятків
...
| Вимога | Файл | Рядки/Методи |
|--------|------|--------------|
| Винятки | `exceptions.py` | Всі класи |
| Генератори | `generators.py` | level_parser, highscore_reader |
| Декоратори | `decorators.py` | timer, cache_result, retry |
| Замикання | `decorators.py` | create_score_tracker, retry |
| Послідовності | `snake.py` | __len__, __getitem__, __iter__ |
| Анотації | Всі файли | Всі функції |
| Пакети | `__init__.py` | У кожній папці |
| Тести | `tests/` | 57 тестів |
